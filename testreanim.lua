...
if Humanoid.RigType == Enum.HumanoidRigType.R15 then
    game.StarterGui:SetCore("ChatMakeSystemMessage", {
        Text = "[Reanimate] You are on R15! Please use the R15 Reanimation instead.",
        Color = Color3.fromRGB(255, 10, 70),
        TextSize = 18,
        Font = Enum.Font.Ubuntu
    });
return end
if game.Players.LocalPlayer.Character:FindFirstChild("Reanimate") or workspace:FindFirstChild("Reanimate") then
    game.StarterGui:SetCore("ChatMakeSystemMessage", {
        Text = "[Reanimate] You are already reanimated! Please reset to unreanimate.",
        Color = Color3.fromRGB(255, 10, 70),
        TextSize = 18,
        Font = Enum.Font.Ubuntu
    });
...
game:WaitForChild("Run Service").Heartbeat:Connect(function()
    if Reanimated == true then
        if Character.Reanimate then
            Character.Torso.CFrame=Character.Reanimate.Torso.CFrame
        end
...
CopyCharacter.Name = "Reanimate" -- you can always change name.

for k,l in pairs(Character.Reanimate:GetDescendants()) do 
...
game:GetService("UserInputService").JumpRequest:connect(function(t)
    if CopyCharacter.Humanoid.FloorMaterial~=Enum.Material.Air then 
        ...
        game.Players.LocalPlayer.Character.Reanimate:FindFirstChildOfClass('Humanoid').Sit=false 
    end 
end)
